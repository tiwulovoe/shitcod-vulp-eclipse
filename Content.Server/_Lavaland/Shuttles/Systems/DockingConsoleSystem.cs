...
public sealed class DockingConsoleSystem : SharedDockingConsoleSystem
{
    ...
    public override void Initialize()
    {
            ...
            subs =>
        {
            ...
            subs.Event<DockingConsoleBeaconFTLMessage>(OnBeaconFTL);
    ...
    public void UpdateUI(Entity<DockingConsoleComponent> ent)
    {
        ...
        List<DockingDestination> beacons = new();
        ...
        if (TryComp<DockingShuttleComponent>(shuttle, out var docking))
        {
            ...
            beacons = GetBeaconDestinations(docking);
        }

        var state = new DockingConsoleState(ftlState, ftlTime, destinations, beacons);
    ...
    private void OnBeaconFTL(Entity<DockingConsoleComponent> ent, ref DockingConsoleBeaconFTLMessage args)
    {
        if (ent.Comp.Shuttle is not { } shuttle || !TryComp<DockingShuttleComponent>(shuttle, out var docking))
            return;

        var beacons = GetBeaconDestinations(docking);
        if (args.Index < 0 || args.Index >= beacons.Count)
            return;

        var dest = beacons[args.Index];
        if (dest.Map == Transform(shuttle).MapID)
            return;

        if (FindDockingGrid(dest.Map, docking.DockTag) is not { } grid)
            return;

        Log.Debug($"{ToPrettyString(args.Actor):user} is FTL-docking {ToPrettyString(shuttle):shuttle} to fixed beacon dock {ToPrettyString(grid):grid}");
        _shuttle.FTLToDock(shuttle, Comp<ShuttleComponent>(shuttle), grid, priorityTag: docking.DockTag);
    }

    private List<DockingDestination> GetBeaconDestinations(DockingShuttleComponent docking)
    {
        var beacons = new List<DockingDestination>();

        foreach (var destination in docking.Destinations)
        {
            if (FindDockingGrid(destination.Map, docking.DockTag) == null)
                continue;

            beacons.Add(destination);
        }

        return beacons;
    }

    private EntityUid? FindDockingGrid(MapId map, string? dockTag)
    {
        ...
        {
            if (xform.MapID != map || xform.GridUid == null)
                continue;

            if (dockTag != null && priority.Tag?.Equals(dockTag) != true)
                continue;
    ...
    private void OnCallShuttle(Entity<DockingConsoleComponent> ent, ref DockingConsoleShuttleCheckMessage args)
    {
        ...
        var shuttleUid = shuttle.Value;
        var shuttleComp = docking;
        if (shuttleComp.Station == null)
        {
            ...
            RaiseLocalEvent(shuttleUid, new ShuttleAddStationEvent(targetUid.Value, targetMap), false);
        }
